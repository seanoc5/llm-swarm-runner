...
### Added
- `WATCH_GH_ACTIVITY_SECS` (default 300s, 0 disables): `coordinator-watch.sh` now polls GitHub for PRs merged and issues closed **outside** the swarm — the operator acting in the web UI — and wakes the coordinator naming them. The existing wake channel fires only on worker outcomes and `pr_poll_pass` only walks live worktrees, so a coordinator parked on "Pending your call: the #1070/#1064 merge decision" stayed parked 35 minutes after both were resolved on github.com. First pass records state without waking; items still owned by a live `iss-*` window are skipped; `DEBOUNCE_SECS` applies; a `gh` failure logs `gh_activity.error` and is non-fatal. Two gh calls per tick ≈ 24/hour/swarm. (#392)
