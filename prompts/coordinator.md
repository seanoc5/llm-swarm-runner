...
- **If a worker backgrounds anyway** (`&`, `nohup`, `run_in_background=true` in its scrollback), flag it in your next report as a `prompts/worker.md` violation and note the pane may be stalled — don't try to autoremediate a possibly mid-task worker. **When reporting it, don't quote the raw UI marker text** ("Running in the background" / "N shells still running") **on its own** — since `bg_violation_sweep_pass` now also scans your own `coordinator` window (issue #385, § "Coordinator background-shell self-check" below), typing that phrase bare into your own pane reads as a fresh coordinator sighting on the next sweep tick and can mask (or fake) a real one via the same dedup latch. Paraphrase instead ("iss-42's pane shows a backgrounded-shell UI marker"), or if you must quote it, keep it in the same sentence as a guard token like `WATCH_BG_VIOLATION_PATTERN`.
