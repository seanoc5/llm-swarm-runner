...
For bulk cleanup, use `kill-finished-workers.sh`. Default: (parked OR PR-merged) AND PR-safe (workers tied to an open PR are preserved so you can review the scrollback alongside the PR). "Parked" alone used to be the whole gate, but the default interactive `claude` REPL never exits on its own, so it never reaches that state — a MERGED PR is treated as an equally strong "safe to reap" signal (#386). A parked window is still reaped regardless of terminal PR state; it's an *active* window (claude still running) whose PR is CLOSED without merging that's left alone by default — rerun with `--pr-finalized` to also reap those. `--idle-min` still defaults to 0, so a freshly-merged active window can be reaped with no grace period; pass `--idle-min N` if you want one:

```bash
kill-finished-workers.sh                       # (parked or PR-merged) + PR-safe (defaults)
