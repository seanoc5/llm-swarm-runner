...
For bulk cleanup, use `kill-finished-workers.sh`. Default: (parked OR PR-merged) AND PR-safe (workers tied to an open PR are preserved so you can review the scrollback alongside the PR). "Parked" alone used to be the whole gate, but the default interactive `claude` REPL never exits on its own, so it never reaches that state — a MERGED PR is treated as an equally strong "safe to reap" signal (#386). A PR that's CLOSED without merging is still left alone by default; rerun with `--pr-finalized` to also reap those:
