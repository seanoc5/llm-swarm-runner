...
| `swarm-merge.sh` | Resolves the PR for an issue, merges it, and cleans up the local worktree + branch + tmux window in one shot. Refuses on a self-review BLOCK verdict or a migration collision (`migration-collision-check.sh`) unless overridden. `--sweep-only` just runs the local-branch sweep. |
