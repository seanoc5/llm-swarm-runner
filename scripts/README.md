...
| `requeue.sh` | Atomically drops a follow-up task brief into a worker's v2 inbox (file or stdin). Best-effort flags an OPEN target PR with a `SWARM_PENDING_BRIEF: queued` marker comment (issue #375), cleared by `worker-listener.sh` once the brief is delivered. |
