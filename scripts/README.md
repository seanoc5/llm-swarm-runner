...
| `ci-wait.sh` | The correct sequence for waiting on a PR's CI: checks `mergeable`/`mergeStateStatus` first (a `CONFLICTING` PR never gets a `pull_request` run — GitHub can't build the merge-preview ref, so there's nothing to poll for), then does one bounded foreground poll of `gh pr checks` against a hard deadline. Exit codes for green/red/timeout/conflicting so workers stop hand-rolling `until … sleep` loops. |
...
| `kill-worktree.sh` | Removes one worker's worktree, branch, and tmux window — ABANDON-verdict helper. Salvages any unprocessed `inbox`/`processing`/`outbox` briefs first (issue #317) and, when it does, best-effort posts a `SWARM_BRIEF_ORPHANED` marker comment on the reaped branch's PR (issue #375). |
...
| `lint-pr-screen.sh` | Altitude gate for a PR body's screen (everything above the folded appendix): fails on code spans, per-label word budgets, and a ✅ inside a Decide table row. Workers run it before `gh pr ready`; exit 0 = gate passed, 3 = rewrite the screen. |
| `list-own-worktrees.sh` | Prints THIS project's own worker worktree directories, one per line — scoped via `git worktree list` against the project's own repo (issue #357), never a `wt-issue-*` name-glob under a shared flat-grouping parent that a sibling project's swarm can also populate. Thin CLI wrapper around `_load-env.sh`'s `swarm_own_worktree_dirs()`. |
...
| `migration-collision-check.sh` | Detects duplicate Flyway version prefixes and Alembic multi-head DAGs in the union of a PR's base and head branches — the only place a burst-merge collision that each worker's own tests miss can be reliably caught. `--post` publishes the verdict as an idempotent `<!-- SWARM_MIGRATION_GATE: <clean\|collision> -->` marker comment. Run by `swarm-merge.sh` before merging (#294). |
...
| `requeue.sh` | Atomically drops a follow-up task brief into a worker's v2 inbox (file or stdin). Best-effort flags an OPEN target PR with a `SWARM_PENDING_BRIEF: queued` marker comment (issue #375), cleared by `worker-listener.sh` once the brief is delivered. The comment carries a delivery outlook (is anything actually polling this brief?), an excerpt of the brief, and a next-steps block (issue #397; `SWARM_PR_BRIEF_EXCERPT=0` suppresses the excerpt). |
| `gh-token-probe.sh` | Exercises every `gh` operation the swarm's scripts and prompts use against a candidate token (`PROBE_TOKEN`), read-only by default; opt-in write ops (`PROBE_WRITE=1`) run only against a throwaway `PROBE_REPO` and refuse the real swarm repos. Run it before putting a scoped token in `.sandbox-env` (#418). |
...
| `swarm-merge.sh` | Accepts an issue# or a PR# — resolves the other from it (#324) — merges the PR, and cleans up the local worktree + branch + tmux window in one shot. Refuses on a self-review BLOCK verdict or a migration collision (`migration-collision-check.sh`) unless overridden. `--sweep-only` just runs the local-branch sweep. |
