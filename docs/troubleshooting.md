...
`FRESH` (exit 0) means a live watcher whose on-disk script still matches what it was at launch; `STALE` (exit 1) means a live watcher whose script has changed since — re-run `llm-start.sh` (`WATCH=1`, the default, is idempotent about spawning the watcher) to get a fresh process. `NOT RUNNING` (exit 3) means the state file exists but its recorded pid isn't alive — the state file is written once at startup and never removed, so a watcher that already exited (clean shutdown, crash, or its own staleness self-check) leaves a stale-looking file behind; without this liveness check that corpse would otherwise misreport FRESH. Exit 2 means no watcher has ever run for that project dir at all. By default the watcher also self-checks every `WATCHER_STALE_CHECK_SECS` (300s) and shuts itself down on a mismatch, logging `watch.stale_daemon`.

Before issue #405, nothing auto-restarted the daemon afterward — a dead watcher pane after that log line was expected, and a tmux session that had been up since before the watcher died (never getting a fresh `llm-start.sh` run) could stay wake-dead indefinitely; this is exactly what happened to a real swarm (fand-etl went ~2 days without wakes). Now, by default (`WATCHER_STALE_RESPAWN=1`), the shutdown also runs `tmux respawn-pane -k` against its own pane (identified via `$TMUX_PANE`) right before the self-kill — this restarts the pane's original `coordinator-watch.sh` invocation under the now-current code within seconds, no human action needed. A `watch.stale_daemon_respawn` log line confirms that command exited 0 (the pane came back); `watch.stale_daemon_respawn_failed` means it didn't (gone/stale pane id, dead tmux server) and the pane may still be dead. Either way, if the pane is dead and neither respawn line is present (respawn disabled, or not running under tmux at all), fall back to the old recovery: re-run `llm-start.sh`.
