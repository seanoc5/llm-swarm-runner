...
| `WATCH_RECONCILE_NUDGE` | `1` | (issue #451) When `WATCH_CHECK_ON_DONE` sees a done signal with no completion record yet, it always logs `watch.reconcile` and (unless this is `0`) drops a one-time reminder brief into the worktree's `inbox/` pointing the worker at `scripts/task-done.sh`. Never fabricates a `done/*.json` outcome itself — see `scripts/task-done.sh`'s header for why the worker is now the sole writer. |
