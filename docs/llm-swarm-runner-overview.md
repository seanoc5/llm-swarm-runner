...
- **GH token passthrough:** reads `gh auth token` on the host and injects as `GH_TOKEN` (necessary because `gh` stores its token in the system keyring on Linux, not in the mounted config dir). **Scoping (#418):** a `GH_TOKEN=github_pat_…` line in `<project>/.sandbox-env` replaces it for that project's workers — Docker reads the file, the host token is not injected on top, and the session header names the source. Verify a candidate token first with `scripts/gh-token-probe.sh`; permissions and procedure in [gh-token-scoping.md](./gh-token-scoping.md).
...
(issue #317) The auto-reap that removes a worktree can race a `requeue.sh` follow-up brief that hasn't reached the worker yet — `--pr-finalized`/`--merged-only` modes bypass the "listener parked" check specifically so they can reap mid-task, making `.swarm/tasks/processing/` (a claimed-but-unfinished brief) the likeliest place to find in-flight work, not `inbox/` alone. Before removing anything, `kill-worktree.sh` moves any unprocessed file in `inbox/`, `processing/`, or `outbox/` into `<project>/.swarm/salvaged/iss-<N>/{inbox,processing,outbox}/` and prints a `SALVAGED: ...` line, instead of destroying it. `--refuse-nonempty-inbox` (or `SWARM_REAP_INBOX=refuse`) skips the worktree entirely (exit 76) instead, for operators who'd rather the reap stall than salvage-and-remove.

(issue #375) Salvage preserves the brief but not the fact that a human merging the PR from GitHub never saw it queued. When a salvage actually moves files, `kill-worktree.sh` also best-effort posts a `<!-- SWARM_BRIEF_ORPHANED -->` comment on the reaped branch's PR (any state — this fires as the worktree is being destroyed, so the PR is usually already MERGED/CLOSED), naming the salvage dir. Pairs with `requeue.sh`'s `SWARM_PENDING_BRIEF` marker below, which flags the PR the moment a follow-up is queued, before any reap. Both are pure signals: no `gh`, no PR for the branch, or a lookup failure is silently swallowed and never blocks the reap itself.
...
After dropping the brief, prints a hint about whether the listener tmux window is alive — so you don't sit waiting for a brief that nothing is polling. If no listener is running, prints the exact `tmux new-window` / `sandbox.sh listener` command to start one. The same probe (`listener_state`: `live-idle` / `live-busy:<cmd>` / `no-listener` / `no-session`) is rendered a second time as prose in the PR comment below — one probe, two surfaces.

Use for PARTIAL or NEEDS_REVIEW verdicts where a follow-up surgical brief is the right next step.

**issue #375 — PR-side reap-race signal.** A requeued brief can outlive the reap race: a coordinator queues a review-fix follow-up, but the target PR merges from GitHub before the worker ever claims it, and the watcher's `--pr-finalized`/`--merged-only` auto-reap then salvages the now-orphaned brief (issue #317) after the flawed code is already on `master`. Salvage preserves the brief but gives a human merging from GitHub no signal that a fix was in flight. `requeue.sh` closes that gap best-effort: if the target worktree's branch has an OPEN PR, it posts a `<!-- SWARM_PENDING_BRIEF: queued -->` marker comment (idempotent — skipped while the PR's latest such marker already says "queued"). `worker-listener.sh` posts the matching `SWARM_PENDING_BRIEF: cleared` comment once the task draining that brief finishes with a PR resolved in its status file. If the reap still wins the race, `kill-worktree.sh`'s salvage step (above) posts a `SWARM_BRIEF_ORPHANED` comment on the reaped PR pointing at the salvage dir. All three are pure signals — no `gh`, no network, no PR on the branch, or a lookup failure is silently swallowed and never blocks the requeue, the task, or the reap.

**issue #397 — the markers carry next steps, not just a warning.** A `SWARM_PENDING_BRIEF: queued` comment that says only "merging now may ship without this queued fix" leaves the reader — typically in the GitHub web UI, with neither the brief nor a shell on the swarm host — with no way to act on it. Both marker comments now answer the two questions that actually decide the merge:

- **Will waiting work?** `requeue.sh` already probed the listener state to print a terminal hint nobody reads; that verdict is now a `**Delivery outlook at queue time:**` line in the comment itself. A live idle `iss-<N>` window blesses waiting; a session with no `iss-*` window, or no session at all, says **"nothing is polling this brief"** and **"Do not just wait"** — the case where waiting for the `cleared` comment never resolves.
- **Does it matter?** A collapsed `<details>` block carries the head of the brief (default 20 lines, `SWARM_PR_BRIEF_LINES`), so a review BLOCK is distinguishable from a nit without leaving the page. Set `SWARM_PR_BRIEF_EXCERPT=0` to suppress the excerpt and keep the rest. Any run of six-or-more backticks in the brief is defanged so a brief carrying its own code fences cannot break out of the comment's block.

Both then close with a numbered **Next steps** block of copy-pasteable commands. For `SWARM_PENDING_BRIEF: queued`: inspect the queue, merge anyway (with the `gh issue create` step that keeps the fix from being lost), or cancel an obsolete brief. For `SWARM_BRIEF_ORPHANED` — the last signal anyone ever gets about that brief — read the rest of it, re-file it as an issue (or re-provision against the original issue, whose number is threaded into the comment), or drop it so the next reader does not re-litigate it. `scripts/migration-collision-check.sh`'s `SWARM_MIGRATION_GATE` comment, which has always carried explicit `Remediation:` lines, is the bar these were brought up to.
...
| `WATCH_ACTIVITY_POLL_SECS` | `300` | (issue #392) Independent timer that polls `gh pr list --state merged` / `gh issue list --state closed` for activity since the last check and wakes the coordinator naming what changed — the backstop for an operator merging/closing a PR or issue straight in the GitHub web UI for a worker already reaped (no live window/worktree left for `WATCH_PR_POLL_SECS`/`WATCH_ORPHAN_SWEEP_SECS` to find). Skips anything this swarm's own reap pipeline already logged or is still actively reaping (a still-live worktree). `0` disables. |
| `WATCH_CHECK_ON_DONE` | `1` | Runs the project's acceptance check in a visible `chk-N` tmux window the moment a worker signals done (a `status/<id>.json` with `state: ready-for-review`, or a PR appearing with no status file, via the `WATCH_PR_POLL_SECS` backstop). Records the result to `<id>.check.json` and `events.log`; skipped (state=skipped) if the PR is already MERGED/CLOSED by the time the check claim is won. `0` disables. (issue #451) The same done-signal also reconciles against `.swarm/tasks/done/`: if no completion record exists yet for that task, it logs `watch.reconcile` and takes no other action — it never fabricates a `done/*.json` outcome; the worker is the sole writer, via `scripts/task-done.sh`. |
...
5. **Archive + record** — move brief to `done/` (v2) or `.agent-task-last.md` (v1); for v2 also write `done/<id>.{ok,err}.json` with timing, agent exit code, and check result. This step is now usually pre-empted by the worker itself: `scripts/task-done.sh` (issue #451) is the worker's own mandatory last step (`prompts/worker.md` § "Task completion"), called mid-session — this listener step then just recognizes the record already exists and skips writing a duplicate. It's the fallback for a worker that forgets, and the only writer at all for a headless dispatch or a pre-#451 worktree.
