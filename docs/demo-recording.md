...
`edit-demo.sh` reads the `SEGMENTS=(...)` array near the top of its source and stitches the segments together with per-segment speed multipliers. **After your first take, you'll need to scrub through the raw file and update those timestamps** to match the actual beats — there's no auto-detection. Each row is:
