...
Replacement: a single baked-in voice in `prompts/worker.md` § "Worker voice" — status at milestones, options only at genuine decision points. The structural conventions (summary, decision framing, risk rating) were never verbosity-controlled and are unchanged. The original text above stands as history; grep for `WORKER_VERBOSITY` in it refers to the retired mechanism. A companion change reshaping the end-of-task summary blocks into a single `## Handoff` block is tracked as issue #280.
