...
*   **What's exposed:** every line every agent prints — coordinator reasoning, worker tool calls, error messages, and anything claude/gemini echoes from `gh`, `git`, `cat`, etc. — lives in pane scrollback for the life of the session (50000 lines by default; `llm-start.sh` sets this on the swarm socket at startup, mirrored in [`examples/tmux.conf.example`](../examples/tmux.conf.example) for manual use). `remain-on-exit failed` (same source) keeps that scrollback readable after the agent dies. Anyone who can `tmux -L swarm-<repo> attach` (i.e. anyone on the host with your UID, plus root) can read all of it.
