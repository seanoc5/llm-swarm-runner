...
`coordinator-watch.sh` also runs a periodic fallback sweep (`WATCH_BG_VIOLATION_SWEEP_SECS`, on by default) that scans every `iss-*` worker pane and the coordinator's own pane for the background-shell UI markers Claude Code leaves behind ("Running in the background", "N shells still running" at rest — the exact `WATCH_BG_VIOLATION_PATTERN` / `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` markers) — coverage of the coordinator's own pane is [#385](https://github.com/seanoc5/llm-swarm-runner/issues/385); the sweep overall catches an opted-out project, a future harness change, or gemini/codex (which have no equivalent env-var switch for either role; the prompt rule remains their only guard). Delivery differs by role: a worker sighting gets a `fyi` message dropped into that worker's own outbox so the coordinator surfaces it on its next wake; a coordinator sighting has no outbox to drop into (see [#383](https://github.com/seanoc5/llm-swarm-runner/issues/383)/[#384](https://github.com/seanoc5/llm-swarm-runner/issues/384) for why `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` closes this for claude coordinators specifically — this sweep is the backstop for everything else), so it's logged as a `watch.bg_violation` line in `.swarm/events.log` for the coordinator's own per-wake self-check (`prompts/coordinator.md`) to pick up. See `scripts/coordinator-watch.sh`'s header comment for the full knob list.
...
1. **Claude Code's built-in auto-compact defaults to the full model window.** Since Claude Code 2.1.x the window *is* tunable — `/autocompact <tokens>`, the `autoCompactWindow` key in `settings.json`, or the `CLAUDE_CODE_AUTO_COMPACT_WINDOW` env var — but the default for Opus 5.5 / Sonnet 5 is their native 1M, so the cutover (window − 13k) sits near 987k and a worker can cruise through the whole degradation band without it firing. The env var does not reach sandboxed workers (`sandbox.sh` passes no `CLAUDE_*` vars); the settings key does, because `~/.claude` is bind-mounted into every worker. On this host it is pinned at `220000` (cutover ≈207k, summary pre-computed from ≈176k) as of 2026-09-24, and the watcher thresholds below (`AUTO_COMPACT_THRESHOLD_TOKENS`, `WORKER_COMPACT_THRESHOLD_TOKENS`, effective 250k cap) are the backstop rather than the primary trigger. `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` is a test hook that can only *lower* the cutover (percent 1–100; `0.95` means 0.95%, which compacts every turn) — leave it unset.
