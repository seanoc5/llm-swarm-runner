...
`coordinator-watch.sh` also runs a periodic fallback sweep (`WATCH_BG_VIOLATION_SWEEP_SECS`, on by default) that scans every `iss-*` worker pane **and the coordinator's own pane** ([#385](https://github.com/seanoc5/llm-swarm-runner/issues/385)) for the background-shell UI markers Claude Code leaves behind ("Running in the background", "N shells still running" at rest) — this catches an opted-out project, a future harness change, or gemini/codex (which have no equivalent env-var switch for either role; the prompt rule remains their only guard). Delivery differs by role: a worker sighting gets a `fyi` message dropped into that worker's own outbox so the coordinator surfaces it on its next wake; a coordinator sighting has no outbox to drop into (see [#383](https://github.com/seanoc5/llm-swarm-runner/issues/383)/[#384](https://github.com/seanoc5/llm-swarm-runner/issues/384) for why `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` closes this for claude coordinators specifically — this sweep is the backstop for everything else), so it's logged as a `watch.bg_violation` line in `.swarm/events.log` for the coordinator's own per-wake self-check (`prompts/coordinator.md`) to pick up. See `scripts/coordinator-watch.sh`'s header comment for the full knob list.
